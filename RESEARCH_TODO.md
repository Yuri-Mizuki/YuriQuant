...
| **银河 0608 时序截面三层预测（银河系列前篇）** | `[x]` **研读完成（09-20）** | 报告 `reports/docs/research_notes/银河0608_研读_时序截面三层预测与QP口径.md`：特征全公式（~30 个/G1G2G3 分组差异化预处理）、标签三套（MaxDD RankIC 0.179 显著/Sharpe 0.068/Alpha 弃用——收益难测风险好测）、模型 16 维极小 CPU 可行、QP 臂原始口径（主动空间+TE/换手/top5 硬约束+风险中性软惩罚+Σ20% 收缩）全给 |
