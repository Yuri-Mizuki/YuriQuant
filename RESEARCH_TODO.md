...
- [ ] **factor/classic 特征扩展（银河 0608 转译，09-20 新增，一天级）**
  —— **= 复现 L1 第一步、Phase 2 信号地基，排队优先级提升**：
...
- [x] ~~**stage2 组合优化 RL 实现——Phase 1（hs300 平价验证）**~~ —— **完成（09-20）**
  ...
  - **Phase 2 前置（= 银河 0608 复现 L1，09-20 定稿，本机 1-2 天）**：
    ① `factor/classic` 补 ~30 特征（见下方待办）；② 三标签 GBDT
    （alpha_22 / sharpe_22 / **mdd_66**，训练段估计 + 验证集 RankIC>0.05
    质量门槛 + alpha 回退 sharpe 的降级机制）——这同时验证研报核心结论
    「收益难测、风险好测」（研报参照数字：MaxDD 0.179 / Sharpe 0.068，
    GBDT 达同量级即认定风险标签可测）。L1 不是独立复现，是 Phase 2 的
    信号设计本身。
  - **银河 0608 复现 L2（三层网络）后置**：CPU 无算力墙，但边际价值存疑
    （研报自证增益主要来自特征+标签+门槛而非网络结构，Sharpe RankIC 仅
    0.068）。**触发条件**：L1 的 GBDT 在同特征同标签下 Sharpe/MaxDD
    RankIC 明显低于研报量级（截面联动的增量），才立项；否则归档。
  - **Phase 2 待办（zz1000 主实验）**：信号用 L1 产出（f=alpha_z+sharpe_z
    合成、z=MaxDD 预测）；滚动训练协议（逐年 + 双窗口双门槛 + 多种子集成
    + 回退）；银河 6 组消融；结果接 `stats/pbo.py`。预计大算力，随长实验
    同机排队。
