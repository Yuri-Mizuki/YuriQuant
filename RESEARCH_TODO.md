...
- [~] **西南 T2RL** —— **研读完成（2026-09-18），复现未开始**
  ...
  - **建议路线（stage2-only，性价比最高）**：项目 GBDT h=10 预测替代 TFAC 因子，
    SB3 SAC（`rl` extra 已含）+ 自建组合层 env（复用 `alphapool_env` 成交约束经验）；
    stage1 不复现（`model/` 已有等价物）。研报数字口径（费率 0.00025、开盘价成交）
    与项目 C0 不可比
...
## 二、P1 高度参考（6 篇；2026-09-18 排定研读优先级）

- [ ] **银河 0706 基于深度学习预测与强化学习优化的指数增强**（20260706，`强化学习/`）
  —— **下一批研读首位**：与 `optimize/` 工程形态最接近；研读时与 T2RL 并排比对
  三轴（预测模型 / RL 算法 / 约束处理），产出直接决定 T2RL stage2 复现的最终设计
- [ ] **华安 226 风险规避型强化学习模型在投资组合优化中的应用**（20250305，`强化学习/`）
  —— **与银河 0706 同批**（凑齐 RL 组合优化全景）；对接 `optimize/risk.py`
- [ ] **招商证券 因子筛选与投资组合构建**（20181023，`因子合成/`）—— 优化层约束规范，速读校准
- [ ] **江海证券 机器学习在多因子组合中的应用**（20240912，`因子合成/`）—— 速读
- [ ] **国金 大模型赋能投研系列（4 篇）**（20260430–20260723，`大模型投研/`）
  —— 工具链方向，非 alpha 来源；与项目新起的邮件日报链路
  （`scripts/reporting/build_daily_email_body.py`）直接衔接，穿插速读
- [ ] **华泰 AI43 因子观点融入 ML** —— **提前**：项目已有 BL 基础设施
  （`optimize/solver.py` 的 `bl_posterior`/`bl_views_from_factor`），
  是最顺手的半天级增量；AI30 因果推断保持低优先
...
- [ ] 华泰 AI29 另类标签与集成学习 —— **已提前进短期队列（09-18）**：项目刚做完
  tradable labels 标签层治本，正处标签体系扩展最佳时机，`model/labels.py` 可直接对照
...
| 序 | 任务 | 理由 |
|---|---|---|
| **0** | **跑已就绪长实验（一律挂机，不占人力）**：① AI97 三臂 ×3 seed（命令见上文 AI97 条）→ ② 国金24 残余⑤ 校准轮（1 臂 1 seed）→ 视耗时铺三臂 → ③ T 扫描全量 | 三者代码/前置全就绪，是"临门一脚"项；AI97 跑完接 `stats.pbo` 做过拟合检验 |
| 1 | **银河 0706 + 华安 226 研读**（跑批 ① 期间） | 与 T2RL 并排比对三轴（预测模型/RL 算法/约束处理），凑齐 RL 组合优化全景 → **落定 T2RL stage2 复现最终设计**（GBDT 预测替代 TFAC + SB3 SAC 组合层 env，三臂 SAC vs QP vs 等权） |
| 2 | ~~西南 T2RL~~ **研读完成（09-18）** | 报告见 research_notes；stage2 设计等 1 的并排比对后再定稿 |
| 3 | **国金19 Mamba2：半天成本核算** | 模型层最大短板（30%）；先量 CPU 训练成本——**可行才立项**，不可行则记"数据面不支持"归档 |
| 4 | **穿插：AI43（BL 观点融入）+ AI29（另类标签）** | 各半天，直接挂已有基础设施（`bl_posterior`/`model/labels.py`）；AI29 衔接刚完成的 tradable labels 治本 |
| 5 | **国金大模型投研 4 篇速读** | 工具链方向，衔接邮件日报链路；纯速读不写代码 |
...
- [ ] **AI97 正式实验（长实验，09-18 定稿命令，待跑）**：跑满 10000 步 +
...
| 因子合成 | 9 | 申万 ML 合成、**多因子10 口径核对 + 两主力方法补齐（09-16）+ T 扫描脚本（09-18）** | 国金19 Mamba2、华泰3128、**T 扫描全量 + 多窗口对比（待跑）** | AI28 |
| 文本挖掘 | 6 | **51/57/63/41 全部** | 华泰 LLM_FADT | — |
| 强化学习 | 6 | **西南 T2RL 研读（09-18）** | T2RL stage2 复现、银河0706、华安226、东方 DFQ、DQN | — |
| 组合优化 | 1 | **AI39 三处低成本对齐（09-18）** | 主动空间结构性对齐 + Barra CNE5（均单独立项） | — |
| 传统机器学习 | 7 | AI6/AI11/AI14/AI16、**AI19/AI22 PBO（09-18）** | AI43 | 6（华泰重复归档） |
| 大模型投研 | 4 | — | 4 篇速读 | — |
| 探索 | 1 | — | AI42 | — |
