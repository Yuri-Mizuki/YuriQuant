...
> **2026-09-20 四批**：银河 0608（系列前篇）研读完成——银河系列对项目
> 「可抄密度」最高的一篇（特征全公式/训练细节全给/模型极小 CPU 可行/QP 口径
> 完整）；产出三个 Phase 2 升级项（QP 臂口径/α 合成回退/MaxDD 风险信号）与
> 一个特征工程落地待办。**复现分层定稿**：L1（特征+三标签+门槛进 GBDT）
> 升为 stage2 Phase 2 前置（本机 1-2 天，同时验证「风险好测」结论）；
> L2（三层网络）后置，触发条件=L1 与研报数字有明显差距。
...
| **银河 0608 时序截面三层预测（银河系列前篇）** | `[x]` **研读完成（09-20）** | 报告 `reports/docs/research_notes/银河0608_研读_时序截面三层预测与QP口径.md`：特征全公式（~30 个/G1G2G3 分组差异化预处理）、标签三套（MaxDD RankIC 0.179 显著/Sharpe 0.068/Alpha 弃用——收益难测风险好测）、模型 16 维极小 CPU 可行、QP 臂原始口径（主动空间+TE/换手/top5 硬约束+风险中性软惩罚+Σ20% 收缩）全给。**复现分层定稿（09-20）**：L1（特征+三标签+质量门槛进 GBDT）
= Phase 2 前置高优先；L2（三层网络）后置，触发条件=L1 与研报数字
有明显差距；L3（QP 口径）已并入 Phase 2 升级项 |
...
- [x] ~~**factor/classic 特征扩展（银河 0608 转译）**~~ —— **完成（09-20）**：
  `compute_galaxy_features`（36 特征）+ `build_galaxy_labels`（三标签）
  + 6 用例单测（`tests/test_galaxy_features.py`）。原计划要点：
  按附录表 12 公式补 ~30 特征（重点 G2 风险组 idvol/downvol/mdd/ret_min/max
  与 G1 相对强弱组 excess_mom/beta_24/80——指增语义）+ 分组差异化预处理
  对照实验（方向性特征不标准化 vs 全 zscore）。特征全公式见 research_notes 报告。
...
- [x] ~~**stage2 组合优化 RL 实现——Phase 1（hs300 平价验证）**~~ —— **完成（09-20）**
  ...
  - [x] ~~**Phase 2 前置（= 银河 0608 复现 L1）**~~ —— **完成（09-20，当日落地）**：
    ① `factor/classic` 新增 `compute_galaxy_features`（36 特征，G1/G2/G3
    分组差异化预处理：形态原始尺度、其余 50 期时序 zscore）+
    `build_galaxy_labels`（三标签前视面板）；② `scripts/evaluation/galaxy_l1.py`
    滚动 GBDT 评估（3 年窗 80/20、年更、LightGBM+早停）。**结果**
    （hs300，2024/2025 两个模型年，2026 因数据尽头无测试年空缺）：
    **mdd_66 稳定显著——valid 0.37/0.18、test 0.25/0.40**（研报参照 0.1792，
    量级之内且测试段更高）→ **「风险标签可测」成立**，stage2 的 z 信号
    升级为 MaxDD 预测（GBDT 即可）；**sharpe/alpha 年度翻符号**
    （sharpe valid +0.069/−0.140，alpha +0.092/−0.083）→ 质量门槛+回退
    机制是必要的而非装饰，且必须逐年判断。引用须注明：mdd 可测部分来自
    波动自相关（风险管理信号，≠新 alpha）；日线 mult=1、等权基准代理、
    单 seed。
  ...
  - **Phase 2 升级项（银河 0608 研读产出，09-20）**：① QP 臂口径从简化 mvo
    ...
    ② f 信号采用 `alpha_z + sharpe_z` 合成 + **质量门槛回退**（Alpha 预测
    验证集 RankIC<0.05 时回退 sharpe_z——研报的降级机制）；③ z 信号升级为
    MaxDD 标签的 GBDT 预测（研报实证风险预测显著更稳：RankIC 0.179 vs
    Sharpe 0.068 vs Alpha 弃用）。
...
| 因子挖掘 | 7 | 国金22、国泰君安 GP、**国金24 主体**、**华泰 AI97（P0 环境 + LLM 初始池）**、**银河 0608 研读（09-20）** | AI26、国金24 残余③④ | — |
