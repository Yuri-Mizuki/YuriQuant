...
class Universe:
    ...
    def get_membership_mask(
        ...
        O(天数) 次独立查询：对每条 (con_code, in_date, out_date) 记录，
        标记其覆盖的所有交易日为 True。
        ...
        codes = df["con_code"].unique()
        mask = pd.DataFrame(False, index=dates, columns=codes)

        dates_arr = dates.values.reshape(-1, 1)  # (n_dates, 1)
        for code in codes:
            rows = df[df["con_code"] == code]
            in_dates = rows["in_date"].values.reshape(1, -1)  # (1, n_rows)
            out_dates = rows["out_date"].values.reshape(1, -1)
            covered = (dates_arr >= in_dates) & (
                pd.isna(rows["out_date"].values).reshape(1, -1) | (dates_arr < out_dates)
            )
            mask[code] = covered.any(axis=1)
