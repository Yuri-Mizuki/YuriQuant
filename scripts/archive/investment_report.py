...
> **已归档（2026-09-24）**：零引用入口（仅 mock 测试保活）且依赖
> `e2e_backtest` 四个函数，随其三口同裁移入。
...
用法：
    python scripts/archive/investment_report.py --real --top 50
    python scripts/archive/investment_report.py --real --top 50 --index 000001.SH
    python scripts/archive/investment_report.py --mock --top 20 --model ridge   # 测试
"""
...
def run(args) -> dict:
    ...
    if args.neutralize and args.real:
        from scripts.common.e2e_common import build_neutral_covariates, neutralize_predictions
