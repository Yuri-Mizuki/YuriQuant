...
> **已归档（2026-09-24）**：HS300 时代入口（`daily_hs300.parquet` 停更、
> 产物 09-16 清理）；职责由全A 主实验（`rolling_grid_alla`）与生产入口
> （`alla_daily_rank`）承担。三口同裁：`optimize_e2e` / `investment_report`
> 一并移入本目录；mock 链路测试改指此处保活。
...
用法：
    python scripts/archive/e2e_backtest.py --real --top 50
    python scripts/archive/e2e_backtest.py --real --top 50 --freq W
    python scripts/archive/e2e_backtest.py --mock --top 20 --model ridge   # 测试/演示
"""
...
def walk_forward_predictions(feats, labels, reb_days, common_days, model="gbdt",
    ...
    beta-neutral alpha 从 -8.6% 转 +5.5%）。
    """
