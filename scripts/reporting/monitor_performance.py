...
用法：
    ...
    python scripts/reporting/monitor_performance.py --dataset hs300_2022_2025 --as-of 20251231

    # 首次生产化：把 h=1 模型预测（ml_synthesis 实验 OOS 面板）回写因子库
    python scripts/reporting/monitor_performance.py --register-model-factors

    # 常驻调度（每日 17:30 自动跑一轮，stdlib 循环无额外依赖）
    python scripts/reporting/monitor_performance.py --daemon 17:30

    # 打印 Windows 计划任务注册命令（替代 daemon 的系统级方案）
    python scripts/reporting/monitor_performance.py --task-cmd
...
# 真实数据监控需要自带 AmazingData SDK 与凭证的解释器：
...
def task_scheduler_cmd(dataset: str = "hs300_2022_2025", python_path: str | None = None) -> str:
    ...
    script = (ROOT / "scripts" / "reporting" / "monitor_performance.py").resolve()
