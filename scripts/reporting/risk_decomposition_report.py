...
用法：
    # mock 模式（无需数据源凭证）
    python scripts/reporting/risk_decomposition_report.py --mock

    # 真实数据（需先跑回测产出 BacktestResult 权重历史）
    python scripts/reporting/risk_decomposition_report.py --real --weights-path reports/xxx/weights_history.parquet

    # 自定义参数
    python scripts/reporting/risk_decomposition_report.py --mock --n-days 300 --n-codes 30 --var-quantile 0.05
