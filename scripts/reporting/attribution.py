...
三大归因框架的命令行入口（研究导向，纯 pandas/numpy 实现）：
    ...
    python scripts/reporting/attribution.py fm --mock                  # Fama-MacBeth（mock 演示）
    python scripts/reporting/attribution.py brinson --mock             # Brinson 归因（mock 演示）
    python scripts/reporting/attribution.py ab --mock                  # α/β 分解（mock 演示）

真实面板输入（均为 date×code 面板 CSV，index=date, columns=code）：

    python scripts/reporting/attribution.py fm \
        --factor-csv reports/factor_panel.csv --returns-csv reports/returns_future.csv
    python scripts/reporting/attribution.py brinson \
    ...
    python scripts/reporting/attribution.py ab \
