...
用法:
    python -m scripts.evaluation.walk_forward --mode both        # exhaustive + GP
    python -m scripts.evaluation.walk_forward --mode exhaustive  # 只 exhaustive
    python -m scripts.evaluation.walk_forward --gp-only          # 只 GP
"""
...
log = setup_logging("walk_forward")
...
def load_full_panels(begin: int = 20220101, end: int = 20251231):
    ...
    try:
        from scripts.factors.build_intraday_factors import build_features, minute_frame
