...
数据前置（_base 与因子面板由 rolling_grid_alla 管线维护，本入口直接消费）：
    python scripts/pipelines/rolling_grid_alla.py --stage prep        # _base 基础面板
    python scripts/builders/build_alla_alpha_panels.py ...    # 因子面板（已存在则跳过）

用法:
    python -m scripts.pipelines.run_model_portfolio                       # 全流程（当年预测+回测+今日选股）
    python -m scripts.pipelines.run_model_portfolio --frac 0.20           # 覆盖持仓比例
    python -m scripts.pipelines.run_model_portfolio --refresh-base        # 先重建 _base 再跑
    python -m scripts.pipelines.run_model_portfolio --no-train            # 复用上次预测面板（只回测/选股）
"""
...
from scripts.common.portfolio_common import neutralize_panel  # noqa: E402
...
def build_ensemble_panel(cfg: dict, base: dict, force_retrain: bool):
    ...
    """
    import scripts.pipelines.rolling_grid_alla as RG
...
def main():
    ...
    if args.refresh_base:
        from scripts.pipelines.rolling_grid_alla import stage_prep
        stage_prep()
