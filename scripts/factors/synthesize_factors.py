...
用法:
    ...
    python scripts/factors/synthesize_factors.py
    # 真实数据：先 python -m scripts.ingest.update_data
    python scripts/factors/synthesize_factors.py --real --topk 10 --method all
    # 直接基于已有挖掘结果 CSV
    python scripts/factors/synthesize_factors.py --from reports/factor_mining_xxxx.csv --topk 8
"""
...
def main():
    ...
    if args.save_library:
        ...
        log.info("复合因子入库完成（%d 个，库内 IC 基于训练段），"
                 "可用 `python scripts/factors/mine_factors.py --use-library` 参与下一轮",
                 len(panels_out))
