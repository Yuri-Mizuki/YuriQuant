...
用法
----
    python -m scripts.factors.build_intraday_factors --offline            # 读缓存（推荐）
    python -m scripts.factors.build_intraday_factors --mock               # mock 验证
    python -m scripts.factors.build_intraday_factors --offline --factors rv,overnight_ret  # 子集
    python -m scripts.factors.build_intraday_factors --offline --dataset hs300_2025 --no-save  # 只算不入库
"""
...
def main():
    ...
    if args.no_save:
        print_no_save(names, features)
        return

    lib = FactorLibrary(dataset=dataset)
    log.info("入库到数据集: %s", dataset)
    reg_rows = register_panels(
        lib, features, FACTOR_DEFS, returns_panel,
        source=f"intraday:build_intraday_factors:{PERIOD}min", names=names,
    )
