...
用法:
    python scripts/portfolio/multi_period_backtest.py                     # mock 月度（默认）
    python scripts/portfolio/multi_period_backtest.py --freq M --method mvo
    python scripts/portfolio/multi_period_backtest.py --freq W --method min_var
    python scripts/portfolio/multi_period_backtest.py --freq M --allow-short --short-limit 0.3

    # 真实数据（需先 python -m scripts.ingest.update_data 拉日线/行业/股本；PIT 并集池）
    python scripts/portfolio/multi_period_backtest.py --real --begin 20230101 --end 20241231
    python -m scripts.portfolio.multi_period_backtest --real --begin 20250101 --end 20251231 --method tev
...
def build_real(begin: int, end: int, index: str = "000300.SH"):
    ...
    from scripts.portfolio.compare_portfolio_methods import load_real_data
