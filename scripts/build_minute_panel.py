...
用法
----
    python -m scripts.build_minute_panel --offline                      # 面板 + 覆盖统计
    python -m scripts.build_minute_panel --offline --features           # 另存统计特征长表
    python -m scripts.build_minute_panel --offline --features --demo-ic # 打印特征次日IC快照
    python -m scripts.build_minute_panel --mock                         # 合成数据端到端演示
"""
...
def demo_ic(features: dict[str, pd.DataFrame], cache: "DataCache", pool: str) -> pd.DataFrame:
    ...
    if daily is None or daily.empty:
        raise SystemExit("daily 缓存缺失，--demo-ic 需要先 python -m scripts.update_data")
    ...
    if args.mock:
        ...
        if minute is None or minute.empty:
            raise SystemExit(
                ...
                f"`python -m scripts.update_data` 拉取分钟数据。")
