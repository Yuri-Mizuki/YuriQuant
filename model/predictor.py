...
``rolling_oos``：生产 walk-forward 上线（test 段等分折、每折用折前全部历史
− embargo 隔离带训练），复用 ``factor.cv.forward_roll_folds`` 的唯一合法前推纪律。
"""
...
from factor.cv import forward_roll_folds, make_folds
...
# ---------------------------------------------------------------------------
# 生产 walk-forward 滚动训练（上线唯一合法前推纪律）
# ---------------------------------------------------------------------------
def rolling_oos(
    predictor_cls: type[BasePredictor],
